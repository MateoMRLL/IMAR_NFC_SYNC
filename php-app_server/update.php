...
switch ($resource) {
    ...
    case 'user':
        if (!isset($payload['name'], $payload['email'])) {
            respond(false, 'Payload must contain name and email', $payload);
        }

        $localUuid = $payload['local_uuid'] ?? null; // optional
    ...
    case 'tag':
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO Tags (uid) VALUES (:uid)");
    ...
    case 'assign':
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                SELECT tag_id, BIN_TO_UUID(user_id, 1) AS cloud_uuid, assigned_at
                FROM TagAssignments
                WHERE tag_id = :tag_id
                LIMIT 1
            ");
    ...
    case 'scan':
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                SELECT tag_id, BIN_TO_UUID(user_id, 1) AS cloud_uuid, scanned_at
                FROM ScanLogs
